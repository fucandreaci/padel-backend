...
public class PrenotazioneControllerTest {
    ...
    public void successGetPrenotazioni() throws Exception {
        Utente user = Stub.getUtenteStub();
        given(utenteService.findFromJWT()).willReturn(user);
        given(prenotazioneService.getPrenotazioniByUtente(any())).willReturn(List.of(Stub.getResponsePrenotazioneWithTypeDtoStub()));

        mvc.perform(MockMvcRequestBuilders.get("/prenotazioni")
